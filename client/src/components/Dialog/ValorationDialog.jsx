    ...
    return (
        <Dialog fullWidth open={props.open} onClose={props.close} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" >
            <DialogTitle id="form-dialog-title">{props.title}</DialogTitle>
                <DialogContent>
                    <TextField autoFocus margin="dense" id="punctuation" label="Valoració del 0 al 5" type="number" fullWidth InputProps={{ inputProps: { min: 0, max: 5 } }} onChange={(e) => setPunctuation(e.target.value)}/>
                    <TextField autoFocus margin="dense" id="punctuation" label="Comentari" type="text" fullWidth rows={2} onChange={(e) => setComment(e.target.value)} />
                </DialogContent>
                <DialogActions>
